...
class UserController {
  ...
  markAddressAsDefault = async (
    ...
    const userId = req.user?.id;
    try {
      const updatedAddress = await this.userService.markAddressAsDefault(
        userId as string,
        id
      );
      res.status(STATUS_CODE.OK).json({
  ...
  sendPhoneNumberOTP = async (
    ...
    const userId = req.user?.id;
    try {
      const message = await this.userService.sendPhoneNumberOTP(
        userId as string,
        phoneNumber
      );
      if (message.sid) {
  ...
  verifyPhoneNumberOTP = async (
    ...
    const userId = req.user?.id;
    try {
      const updatedUser = await this.userService.verifyOTP(
        userId as string,
        otp
      );
      res.status(STATUS_CODE.OK).json({
  ...
  resendPhoneNumberOTP = async (
    ...
    const userId = req.user?.id;
    try {
      const message = await this.userService.resendPhoneNumberOTP(
        userId as string
      );
      if (message.sid) {
        res.status(STATUS_CODE.OK).json({
  ...
  sendEmailOTP = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const userId = req.user?.id;
    try {
      const emailResponse = await this.userService.sendEmailOTP(
        userId as string,
        email
      );
      if (emailResponse) {
  ...
  verifyEmailOTP = async (req: Request, res: Response, next: NextFunction) => {
    ...
    console.log(otp);
    try {
      const updatedUser = await this.userService.verifyEmailOTP(
        userId as string,
        otp
      );
      res.status(STATUS_CODE.OK).json({
  ...
  resendEmailOTP = async (req: Request, res: Response, next: NextFunction) => {
    const userId = req.user?.id;
    try {
      const emailResponse = await this.userService.resendEmailOTP(
        userId as string
      );
      if (emailResponse) {
        res.status(STATUS_CODE.OK).json({
